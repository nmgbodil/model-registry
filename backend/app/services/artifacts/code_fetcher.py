"""Helpers for fetching code repositories into temporary working directories."""
...
def _extract_tarball(url: str, headers: dict[str, Any], dest: Path) -> None:
    ...
    try:
        ...
        if response.status_code == 404:
            raise RuntimeError(f"Specified repo, branch or tag does not exist: {url}")
        raise RuntimeError(f"HTTP {response.status_code} for tarball: {url}")
    except requests.RequestException as e:
        raise RuntimeError(f"Network error fetching tarball from {url}") from e
