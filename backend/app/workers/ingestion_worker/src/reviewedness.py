"""Utilities to compute repository reviewedness metrics."""
...
def is_code_file(path: str) -> bool:
    """Return True if the given path points to a code file based on extension.
...
def count_total_loc(repo_path: str) -> int:
    """Count the total lines of code in a repository.
...
def count_reviewed_loc_for_file(
    repo_path: str, rel_path: str, reviewed_commits: set[str]
) -> int:
    """Count lines of code in a file that were added via reviewed pull requests.

    Args:
        repo_path: Path to the local repository.
        rel_path: Path to the file relative to the repository root.
        reviewed_commits: Set of commit SHAs that are considered reviewed.
    ...
    for line in result.stdout.splitlines():
        if line and not line.startswith("\t"):
            # header line
            parts = line.split()
            # first header line of a block:
            # "<sha> <orig_lineno> <final_lineno> <num_lines>"
...
def count_reviewed_loc(repo_path: str, reviewed_commits: set[str]) -> int:
    """Count total lines of code added via reviewed pull requests in a repository.
    ...
    for root, _, files in os.walk(repo_path):
        for f in files:
            ...
            reviewed_loc += count_reviewed_loc_for_file(
                repo_path, rel, reviewed_commits
            )
    return reviewed_loc


def calculate_reviewedness_score(reviewed_loc: int, total_loc: int, url: Url) -> float:
    """Calculate the reviewedness score from 0 to 1.

    Args:
        reviewed_loc (int): Lines of code added via pull requests with at least one
            review.
        total_loc (int): The total lines of code in the repo.
        url (Url): Parsed repository URL used to determine eligibility.

    Returns:
        float: The reviewedness score as a float from 0 to 1 or -1 if there is an issue.
            Returns 0.0 if total_loc is 0 and -1.0 if the repo URL is unusable or not a
            model repository.
    """
    if total_loc == 0:
        score = 0.0
