"""Compute lineage graphs and a treescore metric for registered models."""
...
class LineageGraph:
    """Lineage graph limited to models currently available in this registry.
...
def _normalize_model_ref(value: str) -> str:
    ...
    Currently strips whitespace and drops trailing slashes.
    """
    v = value.strip()
    while v.endswith("/"):
        v = v[:-1]
...
def extract_parents_from_config(config: Dict[str, Any]) -> List[str]:
    ...
    Supports multiple key conventions (HuggingFace-style and custom). If multiple
    keys are present, we union them.

    Returns:
        List of normalized, de-duplicated parent references (preserving order).
    """
    parents: List[str] = []
    seen: Set[str] = set()
...
def load_config_json(
    repo_path: str, filename: str = "config.json"
...
def build_lineage_graph(
    ...
    *,
    ...
    """Build a lineage graph for a model using only currently uploaded models.

    Args:
        model_id: Target model's registry ID.
        get_model_config: Callable returning a model's config dict or None.
        model_exists: Callable indicating whether a model is uploaded.
        max_depth: Safety cap to prevent infinite loops/pathological graphs.

    Returns:
        LineageGraph rooted at model_id with parent edges limited to existing models.
    """
    parents_by_model: Dict[str, List[str]] = {}
...
def calculate_treescore(
    ...
    *,
    ...
    """Compute treescore as the average total score of direct parents.

    Interpretation:
      - Consider the model's direct parents from the lineage graph.
      - Average the total scores for parents that have a score.
      - If there are no parents (or no parents with scores), treescore is 0.0.

    Args:
        model_id: Model whose parents are evaluated.
        lineage_graph: Graph containing parent relationships.
        get_total_score: Callable to fetch a model's total score.

    Returns:
        Tuple of (treescore_float, details_dict).
    """
...
def compute_lineage_and_treescore(
    ...
    *,
    ...
    """Build lineage graph (limited to current registry) and compute treescore.
