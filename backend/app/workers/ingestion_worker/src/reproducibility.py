"""Helpers to extract and execute demo code blocks for reproducibility scoring."""
...
@dataclass
class RunResult:
    """Outcome of attempting to run a demo snippet or command."""
...
def find_model_card(
    repo_path: str, candidates: Sequence[str] = DEFAULT_MODEL_CARD_CANDIDATES
) -> Optional[str]:
...
def _coerce_output(value: str | bytes | None) -> str:
    """Return a string from possible bytes/str/None outputs."""
    if value is None:
        return ""
...
def _attempt_run_all(repo_path: str, runnable: dict[str, list[str]]) -> RunResult:
    """Run python commands/snippets until one fails or all succeed.

    Tries explicit shell python commands first; otherwise runs fenced python snippets.
    All candidates must succeed; otherwise return the first failure.
    """
    ...
    return RunResult(
        ...
        0,
        "",
        "No runnable demo code blocks found in model card.",
        "no_demo_code",
    )
...
def calculate_reproducibility_score(
    ...
    *,
    ...
    """Compute reproducibility score for a repo by running model card demos.
